...
export default {
    ...
    methods:{
         ...
         * 2.晒选项（item）包含子选项(subs.length>0)，
         * 并且当前item处于一个选中状态下的时候(item.id===selectOption.id)
         * ，展示子选项视图
         * 3.设置选中项为用户点击的item
         */
        onOptionsItemClick(item,index){
            // 如果子选项处于展开状态，则关闭子选项视图
            // 需要标记子选项是否处于一个展开状态isShowSubContent
            ...
            // 2.当前item处于一个选中状态下的时候(item.id===selectOption.id)
